...
export default function SportsArb() {
  ...
  const totalNearExpiry = arbers.reduce((s: number, w: any) => s + (w.nearExpiryCount ?? 0), 0);
  const totalVolume     = arbers.reduce((s: number, w: any) => s + (w.nearExpiryVolume ?? 0), 0);
  ...
  const topScore = arbers.length ? Math.max(...arbers.map((w: any) => w.s3Score ?? 0)) : 100;
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-surface-1 border border-border rounded-lg p-4">
          ...
          <p className="text-xs text-muted-foreground mb-3">$0.99+ бакет — ядро S3 стратегии</p>
